...
class FacialAuthenticationSystem:
    ...
    def __init__(self, detector_model_path, embedding_model_path, threshold=0.65, max_attempts=3, timeout=10, use_tensorrt=True, precision='fp16', stability_level=1, camera_mode='flat'):
        ...
        Args:
            ...
            camera_mode: Chế độ camera ('flat' cho camera thường, 'fisheye' cho camera fisheye)
        ...
        Args:
            ...
            camera_mode: Camera mode ('flat' for regular camera, 'fisheye' for fisheye camera)
        """
        # Initialize face detector with TensorRT if requested
        self.detector = FaceDetector(detector_model_path, precision=precision)
        ...
        # Camera mode and fisheye correction
        self.camera_mode = camera_mode
        if camera_mode == 'fisheye':
            self.fisheye_corrector = Defisheye(**get_defisheye_params())
            print("Fisheye correction enabled for authentication")
        else:
            self.fisheye_corrector = None
            print("Using flat camera mode for authentication")
